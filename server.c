...
int main(int argc, char **argv)
{
  ...
  while (1)
  {
    ...
    *pclient = client_fd;
    ...
    clientArgs.tclient = *pclient;
    clientArgs.address = (struct sockaddr *)(&caddr);
    clientArgs.adlen = caddr_len;

    pthread_create(&t, NULL, HandleClient,(void*)&clientArgs) ;
...
//void* HandleClient(void* t_fd, struct sockaddr *addr, void* adlen, void* s_family)
void* HandleClient(void* cargs)
{
    info *in = cargs;
    
    int c_fd = in->tclient;
    size_t addrlen = (size_t)in->adlen;
    struct sockaddr *addr = in->address;
  
    printf("worker %d: established connection with client ", gettid());
  ...
  while ( (res = read(c_fd, clientbuf, 1023)) > 0)
  {

    clientbuf[res] = '\0';
    printf("worker %d:", gettid());
    ...
    printf(" # words = %d", word);
    printf(" and # characters = %d \n", letter);
    char client_out[10];
    sprintf(client_out,"%d %d",word,letter);
    
    write(c_fd,client_out,strlen(client_out));
